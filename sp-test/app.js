...
function shell(g,w,h,e){
  ...
  for(let y=Math.max(0,(cy-R)|0);y<Math.min(h,(cy+R+1)|0);y++){
    for(let x=Math.max(0,(cx-R)|0);x<Math.min(w,(cx+R+1)|0);x++){
      ...
      if(r>1) continue;
      // V27b: keep V24 aurora colors, but make shell opacity genuinely
      // curvature-driven.  z=1 at the visual center and 0 at the limb.
      let z=Math.sqrt(Math.max(0,1-r*r));
      let grazing=1-z;
      // Transparent central face; progressively visible toward the limb.
      let q=cl((grazing-.055)/.945,0,1);
      let a=Math.pow(q,1.22);
      if(r<.28) a*=Math.pow(r/.28,2.2);
